...
static void ST7789_SendCommand(uint8_t cmd)
{
    ...
    // ST7789_WaitLastData();
    HAL_SPI_Transmit(&ST7789_SPI_PORT, &cmd, sizeof(cmd), HAL_MAX_DELAY); // а это работает
    ST7789_WaitLastData();
    ST7789_UnSelect();
}
...
/*----------------------------------------------------------------------------*/
void ST7789_DrawRect(uint16_t cStart, uint16_t rStart, uint16_t cStop, uint16_t rStop, uint16_t color)
{
    if (cStart >= ST7789_WIDTH || rStart >= ST7789_HEIGHT || cStop > ST7789_WIDTH || rStop > ST7789_HEIGHT)
        return;
                        ...
                        uint16_t color, uint16_t bg_color, uint8_t font_size)
{
    ...
    ST7789_SetAddressWindow(cStart, rStart, cStart + (char_width * font_size) - 1, rStart + font_height - 1);
...
void ST7789_PrintStr(uint16_t cStart, uint16_t rStart, const char *str,
                     uint8_t num, uint16_t color, uint16_t bg_color, uint8_t font_size)
{
    if (font_size == 0)
        font_size = CurrentFontSize;
...
void ST7789_PrintCustomStr(uint16_t cStart, uint16_t rStart, uint8_t *str,
                           uint8_t len, uint8_t pos, uint16_t color, uint16_t pos_color,
                           uint16_t bg_color, uint8_t font_size)
{
    ...
    for (int i = 0; i < len; i++)
    {
        if (i == pos)
        {
            ...
            ST7789_PrintSymbol(cStart, rStart, str[i], pos_color, bg_color, font_size);
        }
        else
        {
            ST7789_PrintSymbol(cStart, rStart, str[i], color, bg_color, font_size);
...
/*----------------------------------------------------------------------------*/
void ST7789_PrintStrFieldWidth(uint16_t cStart, uint16_t rStart, const char *str,
                               uint8_t len, int8_t WidthField, uint8_t Symbol,
                               uint16_t color, uint16_t bg_color, uint8_t font_size)
{
    if (font_size == 0)
        font_size = CurrentFontSize;
    ...
    if (WidthField > 0 && len <= WidthField)
    {
        ...
        ST7789_PrintStr(cStart, rStart, NewStr, WidthField, color, bg_color, font_size);
    }
...
/*----------------------------------------------------------------------------*/
void ST7789_PrintNumberFieldWidth(uint16_t cStart, uint16_t rStart, uint16_t Number,
                                  int8_t WidthField, uint8_t Symbol,
                                  uint16_t color, uint16_t bg_color, uint8_t font_size)
{
    if (font_size == 0)
        font_size = CurrentFontSize;
    ...
    {
        char zero[] = "0";
        ST7789_PrintStrFieldWidth(cStart, rStart, zero, 1, WidthField, Symbol, color, bg_color, font_size);
        return;
    }
    ...
    ST7789_PrintStrFieldWidth(cStart, rStart, NumArray, len, WidthField, Symbol, color, bg_color, font_size);
}

/*----------------------------------------------------------------------------*/
void ST7789_EnterSLEEPMode(void)
{
    ST7789_SendCommand(ST7789_SLPIN);
}
