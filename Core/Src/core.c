...
bool ValueChanged = false; // чисто для экрана, к таймерам и и взаимодействию с ними отношение не имеет
...
uint32_t last_counted_freq = 0; // чтоб первый counted != last_counted даже если частота = 0, проходил условие
...
bool IsChanged()
{
    ...
    {
        if (!(FreqArray[i] == ParametersBuffer[i] && HEArray[i] == ParametersBuffer[i + 4] &&
              LEArray[i] == ParametersBuffer[i + 8] && INArray[i] == ParametersBuffer[i + 12]))
        {
            return true;
        }
    }
...
void LoadParametersFromProfile()
{
    for (uint8_t i = 0; i < 3; i++)
    {
        ...
        {
            if (IsExternalSource)
                ST7789_PrintStr(SECOND_POSITION, 2, FreqArray, 4, EXT_DEFAULT_FONT_COLOR, BACKGROUND_COLOR, 0);
        ...
        if (IsExternalSource)
        {
            StopTimers();
            ST7789_Fill_Color(BACKGROUND_COLOR);
            ST7789_PrintStr(SECOND_POSITION, 2, FreqArray, 4, EXT_DEFAULT_FONT_COLOR, ERROR_DEFAULT_FONT_COLOR, 0);
            break;
        }
        StateLoadError = true;
    }
    ...
    if (StateLoadError)
    {
        ...
        ST7789_PrintStr(2, 2, "SAVE..ERR", 9, DefaultFontColor, BACKGROUND_COLOR, 3);
        ST7789_PrintStr(2, 52, "ERASE..MEM", 10, DefaultFontColor, BACKGROUND_COLOR, 3);
...
void CoreInit(
    I2C_HandleTypeDef *_phi2c2,
    TIM_HandleTypeDef *_phtim1,
    TIM_HandleTypeDef *_phtim2,
    TIM_HandleTypeDef *_phtim4)
{
    phi2c2 = _phi2c2;
    phtim1 = _phtim1;
    phtim2 = _phtim2;
    ...
    EEPROM_Init(&eeprom, phi2c2, I2C1_DEVICE_ADDRESS, 256, 1, 8, I2C_MEMADD_SIZE_8BIT);
    ST7789_Init();
...
void TryToSetStateParams()
{
    ...
    if (!StateParamsError)
    {
        if (IsExternalSource) // при каждом изменнении будет печаться, ходя лучше печатать, когда убрали ошибку
        // но тогда надо будет вводить буферную переменную LastStateParamsError...., подумать короче
        {
            ST7789_PrintStr(SECOND_POSITION, 2, FreqArray, 4, EXT_DEFAULT_FONT_COLOR, BACKGROUND_COLOR, 0);
            SetTimers();
            LaunchTimers();
...
void LaunchTimers(void)
{
    ...
    if (LERequestNum != 0)
    {
        TIM4->CCER |= (TIM_CCER_CC1E | TIM_CCER_CC2E); // Включить выходы каналов 1 и 2
...
void PrintScreen()
{
    ...
    if (!IsExternalSource)
    {
        ...
        foo1();
    ST7789_PrintStr(FIRST_POSITION, 57, "HE", 2, DefaultFontColor, BACKGROUND_COLOR, 0);

    ST7789_PrintStr(SECOND_POSITION, 57, HEArray, 4, DefaultFontColor, BACKGROUND_COLOR, 0);
...
void UpdateScreenAfterUp()
{
    ...
    case HZ:
        ...
        ST7789_DrawRect(0, 0, ST7789_WIDTH, FONT_SIZE_1 * 5 + PIXELS_HIGHLIGHT, BG_CURRENT_STATE_COLOR); // PIXELS_HIGHLIGHT пикселей вверх и вниз от строки
        ST7789_PrintStr(FIRST_POSITION, 2, "HZ", 2, CURRENT_STATE_COLOR, BG_CURRENT_STATE_COLOR, 0);
        ST7789_PrintStr(SECOND_POSITION, 2, FreqArray, 4, CURRENT_STATE_COLOR, BG_CURRENT_STATE_COLOR, 0);

        // очищаем старую
        ST7789_DrawRect(0, 57 - PIXELS_HIGHLIGHT, ST7789_WIDTH, 57 + FONT_SIZE_1 * 5 + PIXELS_HIGHLIGHT, BACKGROUND_COLOR); // PIXELS_HIGHLIGHT пикселей вверх и вниз от строки
        ST7789_PrintStr(FIRST_POSITION, 57, "HE", 2, DefaultFontColor, BACKGROUND_COLOR, 0);
        ST7789_PrintStr(SECOND_POSITION, 57, HEArray, 4, DefaultFontColor, BACKGROUND_COLOR, 0);
        break;
    case HE:
        ST7789_DrawRect(0, 57 - PIXELS_HIGHLIGHT, ST7789_WIDTH, 57 + FONT_SIZE_1 * 5 + PIXELS_HIGHLIGHT, BG_CURRENT_STATE_COLOR);
        ST7789_PrintStr(FIRST_POSITION, 57, "HE", 2, CURRENT_STATE_COLOR, BG_CURRENT_STATE_COLOR, 0);
        ST7789_PrintStr(SECOND_POSITION, 57, HEArray, 4, CURRENT_STATE_COLOR, BG_CURRENT_STATE_COLOR, 0);

        ST7789_DrawRect(0, 112 - PIXELS_HIGHLIGHT, ST7789_WIDTH, 112 + FONT_SIZE_1 * 5 + PIXELS_HIGHLIGHT, BACKGROUND_COLOR);
        ST7789_PrintStr(FIRST_POSITION, 112, "LE", 2, DefaultFontColor, BACKGROUND_COLOR, 0);
        ST7789_PrintStr(SECOND_POSITION, 112, LEArray, 4, DefaultFontColor, BACKGROUND_COLOR, 0);
        break;
    case LE:
        ST7789_DrawRect(0, 112 - PIXELS_HIGHLIGHT, ST7789_WIDTH, 112 + FONT_SIZE_1 * 5 + PIXELS_HIGHLIGHT, BG_CURRENT_STATE_COLOR);
        ST7789_PrintStr(FIRST_POSITION, 112, "LE", 2, CURRENT_STATE_COLOR, BG_CURRENT_STATE_COLOR, 0);
        ST7789_PrintStr(SECOND_POSITION, 112, LEArray, 4, CURRENT_STATE_COLOR, BG_CURRENT_STATE_COLOR, 0);

        ST7789_DrawRect(0, 167 - PIXELS_HIGHLIGHT, ST7789_WIDTH, 167 + FONT_SIZE_1 * 5 + PIXELS_HIGHLIGHT, BACKGROUND_COLOR);
        ST7789_PrintStr(FIRST_POSITION, 167, "IN", 2, DefaultFontColor, BACKGROUND_COLOR, 0);
        ST7789_PrintStr(SECOND_POSITION, 167, INArray, 4, DefaultFontColor, BACKGROUND_COLOR, 0);
        break;
    case IN:
        ST7789_DrawRect(0, 167 - PIXELS_HIGHLIGHT, ST7789_WIDTH, 167 + FONT_SIZE_1 * 5 + PIXELS_HIGHLIGHT, BG_CURRENT_STATE_COLOR);
        ST7789_PrintStr(FIRST_POSITION, 167, "IN", 2, CURRENT_STATE_COLOR, BG_CURRENT_STATE_COLOR, 0);
        ST7789_PrintStr(SECOND_POSITION, 167, INArray, 4, CURRENT_STATE_COLOR, BG_CURRENT_STATE_COLOR, 0);

        // очищаем полностью профили
        ST7789_DrawRect(0, ROW_PROFILE - PIXELS_HIGHLIGHT, ST7789_WIDTH, ROW_PROFILE + FONT_SIZE_1 * 5 + PIXELS_HIGHLIGHT, BACKGROUND_COLOR);
        ST7789_PrintStr(FIRST_POSITION + 8, ROW_PROFILE, "1", 1,
                        DefaultFontColor, BACKGROUND_COLOR, 0);
        ...
        case P1:
            ST7789_PrintStr(FIRST_POSITION + 8, ROW_PROFILE, "1", 1,
                            CURRENT_PROFILE, BACKGROUND_COLOR, 6); // на 1 размер больше обычного
            break;
        case P2:
            ST7789_PrintStr(FIRST_POSITION + 63, ROW_PROFILE, "2", 1,
...
void UpdateScreenAfterDown()
{
    ...
    case HE:
        ...
        if (!IsExternalSource)
        {
            ST7789_DrawRect(0, 0, ST7789_WIDTH, FONT_SIZE_1 * 5 + PIXELS_HIGHLIGHT, BACKGROUND_COLOR); // PIXELS_HIGHLIGHT пикселей вверх и вниз от строки
            ST7789_PrintStr(FIRST_POSITION, 2, "HZ", 2, DefaultFontColor, BACKGROUND_COLOR, 0);
            ST7789_PrintStr(SECOND_POSITION, 2, FreqArray, 4, DefaultFontColor, BACKGROUND_COLOR, 0);
        }
        // обновляем новую
        ...
        break;
    case LE:
        ST7789_DrawRect(0, 57 - PIXELS_HIGHLIGHT, ST7789_WIDTH, 57 + FONT_SIZE_1 * 5 + PIXELS_HIGHLIGHT, BACKGROUND_COLOR);
        ST7789_PrintStr(FIRST_POSITION, 57, "HE", 2, DefaultFontColor, BACKGROUND_COLOR, 0);
        ST7789_PrintStr(SECOND_POSITION, 57, HEArray, 4, DefaultFontColor, BACKGROUND_COLOR, 0);
    ...
    case PROFILE:
        ...
        ST7789_DrawRect(0, ROW_PROFILE - PIXELS_HIGHLIGHT, ST7789_WIDTH, ROW_PROFILE + FONT_SIZE_1 * 5 + PIXELS_HIGHLIGHT, BG_CURRENT_STATE_COLOR);
        ST7789_PrintStr(FIRST_POSITION + 8, ROW_PROFILE, "1", 1,
        ...
        // уже должно быть PrintScreen(), и выполнено UpdateScreen(), профили уже должны быть отрисованы
        break;
    }
}
void UpdateScreenPlaceNumber()
{
    uint16_t color = StateParamsError ? CURRENT_STATE_COLOR : CURRENT_STATE_COLOR;      // пока оставим так
    uint16_t bg = StateParamsError ? ERROR_DEFAULT_FONT_COLOR : BG_CURRENT_STATE_COLOR; // а там посмотрим

    switch (State)
    {
    case HZ:
        ST7789_PrintCustomStr(SECOND_POSITION, 2, FreqArray, 4, 3 - PlaceNumber, color, CURRENT_NUMBER_COLOR, bg, 0);
        break;
    case HE:
        ST7789_PrintCustomStr(SECOND_POSITION, 57, HEArray, 4, 3 - PlaceNumber, color, CURRENT_NUMBER_COLOR, bg, 0);
        break;
    case LE:
...
void foo1()
{
    ST7789_DrawRect(0, 0, ST7789_WIDTH, FONT_SIZE_1 * 5 + PIXELS_HIGHLIGHT, BACKGROUND_COLOR);
...
{
    ST7789_DrawRect(0, 0, ST7789_WIDTH, FONT_SIZE_1 * 5 + PIXELS_HIGHLIGHT, BACKGROUND_COLOR);
    ST7789_PrintStr(FIRST_POSITION, 2, "HZ", 2, DEFAULT_FONT_COLOR, BACKGROUND_COLOR, 0);
    ST7789_PrintStr(SECOND_POSITION, 2, FreqArray, 4, DEFAULT_FONT_COLOR, BACKGROUND_COLOR, 0);
}
...
void HandleButtonSave(void)
{
    ...
    if (!StateParamsError && !StateLoadError)
    {
        ...
        ST7789_DrawRect(FIRST_POSITION + 8 + 55 * Profile + 36, ROW_PROFILE - 18,
                        FIRST_POSITION + 8 + 55 * Profile + 36 + 12, ROW_PROFILE - 8, WHITE);
        ValueChanged = false;
    }
}
